...
#+BEGIN_SRC octave  :var N_disk = 20 S_file="micpositions.png" :noweb yes :results file 
<<setup sampling area>>
<<calc sampling area>>
...
[[file:micpositions.png]]

Setup a sound field for testing puroses, we need sound from all
directions.
#+name: setup sound field 
#+BEGIN_SRC octave  :var S_file="micpositions.png" :noweb yes :results file
%% wave numbers 
...
#+BEGIN_SRC octave  :var S_file="micpatterns.png" :noweb yes :results file
<<setup sampling area>>
<<calc sampling area>>
<<setup sampling positions>>
<<calc sampling positions>>
...
Total is 14491 elements using 260651 bytes
...
Variables in the current scope:
   ...
   Attr Name         Size                     Bytes  Class
   ==== ====         ====                     =====  ===== 
   c    M_OF        25x200                    80000  double
        N_order      1x1                          8  double
        R_mic        1x1                          8  double
        alpha        1x1                          8  double
        ans          1x1                          8  double
        k_F          1x200                     1600  double

Total is 5204 elements using 81632 bytes

#+end_example

*** Documentation                                                     :ignore:
#+name: array response
#+BEGIN_SRC octave :var S_file="arrayresponse_freefield.png" :results file
N_order = 3;
R_mic = 0.001;
...
[[file:arrayresponse_freefield.png]]
...
see [[file:~/Kontor/HSEL/Publikation/Konferenz/Eusipco2009/mkbsp_perfmicarray.m::% :file: <test_tetramic.m>]]

frequency responses [[file:~/Kontor/HSEL/Publikation/Konferenz/Eusipco2009/Vortrag/mkbsp_micarray_freqresp.m::% -*- coding: utf-8 -*-]]

*** Schallfeld
#+name: setup plane waves
#+BEGIN_SRC octave :tangle mkexa_frequencyresponses.m :session freqresp 
%% Vektor mit gemessenen Einfallsrichtungen
D_dir = 5;
Theta_D = pi/2 * ones(1, D_dir);
Phi_D = linspace(0, pi/2, D_dir);
vg_D = vector([ones(1, D_dir); Theta_D(1:D_dir); Phi_D(1:D_dir)], 'spherical');

%% Wellenzahl Wellenzahlvektoren
N_dft = 256;
F_min = 10;
F_max = 100000;
...
#+BEGIN_SRC octave  :tangle mkexa_frequencyresponses.m :session freqresp :var S_file = "micarray_signals.png"  d=1 :results file
figure(1); clf; 
...
#+BEGIN_SRC octave  :tangle mkexa_frequencyresponses.m :session freqresp :var n=1 d=1 S_file = "micarray_hoa.png" :results file
figure(1); clf; 
