...
#+date: 2018-03-14; Revision Time-stamp: <2021-09-11 15:18:12 jbatke>
...
for simulating acoustic problems using Octave or Matlab(tm). We refer to Octave for the rest of the document. 
...
pointing to a location in $\mathbb R^3$.

# - Visit section [[*Installation][Installation]] for setup (directory creation, etc) and
#   installation.
# - [[file:acoustics.org::#overview acoustics][Acoustics]] contains some application examples for booast.
# - [[file:sphericalseries.org][Sphericalseries]] contains some stuff for using spherical series
#   approximations of a sound field, AKA Higher Order Ambisonics.

Parts of booast:
- [[file:acoustics.org][Acoustics]] implements acoustical stuff (plane waves,
  spherical waves);
  
- [[file:microphones.org][Microphones]] simulates microphones;
  
- [[file:ambisonics.org][Ambisonics]] provides tools for simulation of Higher Order Ambisonics;

- [[file:misc.org][Misc]] contains miscellanious tools.

** Usage

The toolbox is entirely implemented using org-mode from emacs (see
[[Installation]]).  It makes use of the babel functions, i.e. the ingest
of [[file:octavetemplates.org]] is necessary. For the time being,
[[file:booast.setup]] does this not automatically.

You can tangle the Octave snippets and export the html documentation
from the main org files.

** Installation
*** Matlab/Octave Installation

1. Check out the git repository to some directory, e.g.
   : git clone https://github.com/badkey/booast
2. Add the path to Octave/Matlab where you cloned booast. E.g.
   : addpath('path/to/booast/');

*** Emacs tangling

Booast is setup using Emacs's org mode. If you want to tangle yourself,
Inside the installation directory, cleaning up everything belonging to
the classes vector and vectorgrid might be necessary:
...
Then create output directories for tangling files:
...
We would like to do some automatic tests like in
https://orgmode.org/worg/org-contrib/babel/intro.html

*** Misc 

This toolbox was recently tested with octave version:
#+begin_src octave
version
#+end_src
...
Some naming conventions for octave variables are used throughout the
documentation. 
...
| N         | constant                                                 |
| n         | variable                                                 |
| n_A       | vector containing $A$ values                             |
| n_AB      | matrix with dimensions $A\times B$                       |
| n_ABC...Z | matrix with dimensions $A\times B\times \ldots \times Z$ |
...
- two parameters imput :: vector positions in a matrix
  ...
  #+BEGIN_SRC octave
    x = rand(3, 4, 6, 8); S_type = 'cartesian';
    v = vector(x, S_type);
    ans = size(v);
  #+END_SRC
...
- one parameter input :: we use vectorgrid as input type
  ...
  #+BEGIN_SRC octave 
    x = linspace(1, 10, 9); y = x; z = x; S_type = 'cartesian';
...
file:@vector/plot.m: plot positions of a vector using a marker

**** Octave definition                                             :internal:

#+BEGIN_SRC octave :tangle @vector/plot.m
<<plot doc[:results ascii :post octave comment(s=*this*)]()>>
<<copyright>>
...
#+name: plot vector test 
#+BEGIN_SRC octave :results file :var S_file="plotvectortest.png" :tangle mist.m
<<vectortest>>
plot(v, '*')
<<save S_file>>
#+END_SRC
#+attr_org: :width 300
#+RESULTS: plot vector test
[[file:plotvectortest.png]]
...
#+call: plot doc[:output html]()

#+name: doc plot vector
#+BEGIN_SRC octave :var S_file="img/docplotvector.png" :results file :exports both
N_x = 10;
M_y = 20;
vg_NM = vectorgrid(linspace(0, 1, N_x), linspace(0, 1, M_y), 0, 'cartesian');
v_NM = vector(vg_NM);
plot(v_NM, '+'); xlabel('x'); ylabel('y'); zlabel('z');
<<save S_file>>
#+END_SRC
#+attr_org: :width 500
#+RESULTS: doc plot vector
[[file:img/docplotvector.png]]
...
*** type
**** Definition                                                    :internal:
#+BEGIN_SRC octave :tangle @vectorgrid/type.m
...
*** reshape
**** Definition                                                    :internal:
get() always shapes the output of the matrix when generating the data
from a vectorgrid. Therefore, the function =reshape()= is identical to =get()=.
...
*** grid
**** Definition                                                    :internal:
#+BEGIN_SRC octave :tangle @vectorgrid/grid.m
function [g1, g2, g3] = grid(st)
  g1 = st.achse{1};
  g2 = st.achse{2};
  g3 = st.achse{3};
#+END_SRC
...
*HINT* meshgrid is restricted to 3D application for the time being!
...
#+name: vectorgrid meshgrid default test
#+begin_src octave :results output
  A = 21; B = 30; 
...
#+RESULTS: vectorgrid meshgrid default test
...
#+RESULTS: meshgrid meshgrid x_BA y_BA
...
#+name: matlab meshgrid x_BA y_BA
#+BEGIN_SRC octave
A = 20; B = 30;
x_A = linspace(-3, 3, A);
y_B = linspace(-2, 2, B);
...
#+name: matlab meshgrid x_BAC y_BAC z_BAC
#+BEGIN_SRC octave
A = 10; B = 20; C = 30;
x_A = linspace(0, 3, A);
y_B = linspace(0, 2, B);
...
=surface= takes a vectorgrid =g_CAB= as input argument and some values to
be displayed as a surface according to the dimensions defined by the
vectorgrid.

If =g_CAB= holds a grid spanning over all 3 dimensions, we need to slice
...
This example illustrates the use of the method =surface= for =vectorgrid=. The section ref:sec:surfacealternatives discusses similar examples using Octave/Matlab's surface function.
...
The function should look like the example in the alternatives section ref:sec:surfacealternatives. Here we define the function the booasty way:
...
Finally the results yields Figure ref:fig:surface_example_2d_3d after
stating
...
#+BEGIN_SRC octave :exports results :tangle surface_example_2d_in_3d.m :results file :var S_file="img/2d_in_3d__spherical_surface_colored_booast.png"
...
#+caption: A surface defined using booast. label:fig:surface_example_2d_3d
#+attr_org: :width 400
#+RESULTS: surface example 2d in 3d figure
[[file:img/2d_in_3d__spherical_surface_colored_booast.png]]

***** 3D in 3D

Using 'dependendSurface' we can show the data as a 3D plane in the 3D space. So we state
#+name: spherical surface example dependendSurface
#+BEGIN_SRC octave
surface(g_AB1, s_AB, [], 'dependendSurface');
#+END_SRC

surface() colors the slice defined by =g_1AB= by values =s_AB=.

#+name: spherical surface example 2d in 3d figure dependendSurface
#+BEGIN_SRC octave :exports results :tangle _spherical_surface_example_2d_in_3d.m :results file :var S_file="img/2d_in_3d_spherical_surface_dependend.png"
  <<surface example g_ABC>>
  <<surface example g_AB1>>
  <<surface example s_AB>>

  figure(1); clf;
  <<spherical surface example dependendSurface>>
  view([30, 40]);
  <<save S_file>>
#+END_SRC
#+attr_org: :width 300
#+caption: A surface defined using booast being *dependend* from the definition space. label:surface_example_2d_3d_dep
#+RESULTS: spherical surface example 2d in 3d figure dependendSurface
[[file:img/2d_in_3d_spherical_surface_dependend.png]]


***** STARTED 3D in 3D coloring

We could also color the surface with an extra parameter.
#+name: spherical surface example dependendSurface colored
#+BEGIN_SRC octave
c_AB = int16(s_AB > 6);
surface(g_AB1, s_AB, c_AB, 'dependendSurface');
#+END_SRC

=surface()= colors the slice defined by =g_1AB= by values =c_AB=.
#+name: spherical surface example 2d in 3d figure dependendSurface colored
#+BEGIN_SRC octave :exports results :tangle _spherical_surface_example_2d_in_3d_colored.m :results file :var S_file="img/2d_in_3d_spherical_surface_dependend_colored.png" :session test
  <<surface example g_ABC>>
  <<surface example g_AB1>>
  <<surface example s_AB>>

  figure(1); clf;
  <<spherical surface example dependendSurface colored>>
  view([30, 40]);
  <<save S_file>>
#+END_SRC
#+attr_org: :width 300
#+caption: colored version
#+RESULTS: spherical surface example 2d in 3d figure dependendSurface colored
[[file:img/2d_in_3d_spherical_surface_dependend_colored.png]]


***** 2D in 3D Spherical

We first create a spherical vectorgrid and then define a function on it.
#+name: spherical surface example g_CAB
#+BEGIN_SRC octave
C = 20; A = 30; B = 40; 
g_CAB = vectorgrid(linspace(0, 1, C), pi*linspace(0, 1, A), pi*linspace(-1, 1, B), 'spherical');
#+END_SRC

#+RESULTS: spherical surface example g_CAB

Now we invent the function. It should be defined for the constant grid value $\theta = \pi/2$, the second value of theta in the grid. Therefore we slice this space from the grid:
#+name: spherical surface example g_1AB
#+BEGIN_SRC octave
g_1AB = g_CAB(2,:,:);
#+END_SRC

The function for demonstration purposes is arbitrary... lets invent one: 
#+name: spherical surface example s_AB
#+BEGIN_SRC octave
r_rtp_AB = get(vector(g_1AB), 'spherical');
s_AB = squeeze(sum(r_rtp_AB, 1));
#+END_SRC

#+RESULTS: spherical surface example s_AB

Finally the results yields Figure ref:fig:surface_example_2d_3d after stating
#+name: spherical surface example surface
#+BEGIN_SRC octave
surface(g_1AB, s_AB');
#+END_SRC

#+RESULTS: spherical surface example surface

so =surface= colors the slice defined by =g_1AB= by values =s_AB=.
#+name: spherical surface example 2d in 3d figure
#+BEGIN_SRC octave :exports results :tangle spherical_surface_example_2d_in_3d.m :results file :var S_file="img/2d_in_3d_surface_colored_booast.png" :session test
  <<spherical surface example g_CAB>>
  <<spherical surface example g_1AB>>
  <<spherical surface example s_AB>>

  figure(1); clf;
  <<spherical surface example surface>>
  view([30, 40]);
  <<save S_file>>
#+END_SRC
...
#+caption: A surface defined using booast. label:fig:surface_example_2d_3d
#+attr_org: :width 400
#+RESULTS: spherical surface example 2d in 3d figure
[[file:img/2d_in_3d_surface_colored_booast.png]]



**** Alternatives using Matlab/Octave directly label:sec:surfacealternatives

As an alternative, we do the examples as tests solely using Octave/matlab's =surface= function.

We use the meshgrid as shown in section ref:meshgridalternatives. A function =z_BA= is defined using the grid information.
...
We want to present $z = f(x, y)$ as a plane in the 3D space. Therefore, we need to
 ...
 2. show the z-plane, use $f(x,y)$ for coloring
    ...
    #+END_SRC
 This results in a figure like shown in Figure ref:fig:alternative_2d_in_3d_surface.
...
#+caption: A 2D surface in 3D space showing a function $f(x,y)$ for $z = 0$ using colors. 
#+caption: label:fig:alternative_2d_in_3d_surface
#+attr_org: :width 300
...
3D in 3D using matlab's/octave's =surface= by spanning a 2D array using two vectors:
#+name: alternative 3d in 3d figure 
...
#+END_SRC
#+attr_org: :width 300
#+caption: 3D surface using =surface()= 
#+RESULTS: alternative 3d in 3d figure
[[file:img/3din3d_plot_surface1.png]]


Using three 2D arrays to do the same:
#+name: alternative 2d 2d 2d
...
#+END_SRC
#+attr_org: :width 300
#+caption: same result as before
#+RESULTS: alternative 2d 2d 2d
[[file:img/3din3dplot_surface2.png]]


same again, rotation by reordering the variables
#+name: alternative 2d 2d 2d reorder
...
#+END_SRC
#+attr_org: :width 300
#+caption: rotation by reordering
#+RESULTS: alternative 2d 2d 2d reorder
[[file:img/3din3dplot_surface3.png]]


same again, different colors from =c_BA=
#+name: alternative 2d 2d 2d color
#+BEGIN_SRC octave :var S_file = "img/3din3dplot_surface4.png" :results file :exports results
<<Beispielfunktion 3D>>
clf;
c_BA = int16(y_BA > 0);
surface(z_BA, x_BA, y_BA, c_BA);
view([30, 40]);
<<save S_file>>
#+END_SRC
#+attr_org: :width 300
#+caption: color changed
#+RESULTS: alternative 2d 2d 2d color
