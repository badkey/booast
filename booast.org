...
#+date: 2018-03-14; Revision Time-stamp: <2020-01-28 22:21:58 jbatke>
...
- Visit section [[*Installation][Installation]] for setup (directory creation, etc) and
  installation.
- [[file:acoustics.org::*Acoustic%20Waves][Acoustics]] contains some application examples for booast.
...
*** rotate
file:@vector/rotate.m: calculates something for given argument

**** Octave definition                                             :internal:

#+BEGIN_SRC octave :tangle @vector/rotate.m
<<rotate doc[:results ascii :post octave comment(s=*this*)]()>>
<<copyright>>
...
rotate one position
#+BEGIN_SRC octave
v_north = vector(0, 1, 0, 'cartesian');
v_east = vector(0, 1, 1, 'cartesian');
v_rotated = rotate(v_north, v_east);
ans = get(v_rotated, 'spherical');
#+END_SRC
...
#+name: doc rotate vector
#+BEGIN_SRC octave :var S_file="img/docrotatevector.png" :results file :exports both
N_x = 9;
M_y = 5;
vg_NM = vectorgrid(linspace(0, 1, N_x), linspace(0, 1, M_y), 0, 'cartesian');
v_NM = vector(vg_NM);
v_rot_L = vector(ones(1,3), zeros(1,3), linspace(pi/2, 3/2*pi, 3), 'spherical');
v_rot_NM = rotate(v_NM, v_rot_L);
hold('off');
plot(v_NM, '*'); xlabel('x'); ylabel('y'); zlabel('z');
axis('square');
hold('on');
...
#+END_SRC
#+attr_org: :width 500
#+RESULTS: doc rotate vector
[[file:img/docrotatevector.png]]
0
...
**** Documenation                                                    :ignore:
    ...
    #+call: rotate doc[:output html]()

*** translate
file:@vector/translate.m: calculates something for given argument


**** Octave definition                                             :internal:


#+BEGIN_SRC octave :tangle @vector/translate.m
<<@vector/translate doc[:results ascii :post octave comment(s=*this*)]()>>
<<copyright>>
...
**** Interface test                                                :internal:
translate one position
#+BEGIN_SRC octave
v_1 = vector(0, 1, 0, 'cartesian');
v_dir = vector(1, 0, 0, 'cartesian');
v_translated = translate(v_1, v_dir);
ans = get(v_translated, 'cartesian');
#+END_SRC
...
#+name: doc translate vector
#+BEGIN_SRC octave :var S_file="img/doctranslatevector.png" :results file :exports both
N_x = 9; M_y = 5; 
vg_NM = vectorgrid(linspace(0, 1, N_x), linspace(0, 1, M_y), 0, 'cartesian');
v_NM = vector(vg_NM);
v_trans_L = vector(1:3, 1:3, 1:3, 'cartesian');
v_trans_NM = translate(v_NM, v_trans_L);
hold('off');
plot(v_NM, '*'); xlabel('x'); ylabel('y'); zlabel('z');
axis('square');
hold('on');
...
#+END_SRC
#+attr_org: :width 500
#+RESULTS: doc translate vector
[[file:img/doctranslatevector.png]]


#+name: @vector/translate doc
#+BEGIN_SRC org
,*** Syntax
...
#+call: @vector/translate doc[:output html]()
...
=vectorgrid=. The section ref:sec:surfacealternatives discusses
similar examples using Octave/Matlab's surface function.
...
ref:sec:surfacealternatives. Here we define the function the booasty way:
...
***** 3D in 3D
Using 'dependendSurface' we can show the data as a 3D plane in the 3D
space. So we state
...
Now we invent some function. It should be defined for $\theta =
\pi/2$. Therefore we slice this space from the grid:
...
#+BEGIN_SRC octave :exports results :tangle spherical_surface_example_2d_in_3d.m :results file :var S_file="img/2d_in_3d_surface_colored_booast.png"
...
**** Alternatives label:sec:surfacealternatives
As an alternative, we do the examples as tests solely using
Octave/matlab's =surface= function.

We use the meshgrid as shown in section ref:meshgridalternatives. A
...
This toolbox was tested with octave version:
#+begin_src octave
version
#+end_src
