...
#+DATE: <2017-11-23 Do> Rev. Time-stamp: <2020-11-30 21:46:09 jbatke>
...
S_type = 'real';
N_order = 5;
v_src_rtp = get(v, 'spherical');
Psi_src_O = sphericalmodematrix(N_order, v_src_rtp(2), v_src_rtp(3), S_norm, S_type);
N_pln_O = sphericalnormalisation(N_order, 'planewave');
A_pln_O = N_pln_O .* Psi_src_O;
k_rtp = get(v, 'spherical');
C_pln_OA = sphericalseries_sfc(A_pln_O, [], r_A, inf, k_rtp(1));
Y_OBC = sphericalharmonic(N_order, theta_B, phi_C, S_norm, S_type);
P_ABC = sphericalseries_isf(C_pln_OA, Y_OBC);

figure(2); clf;
% surface(g_AB1, p_AB1);
surface(g_A1C, real(P_ABC(:,20,:)));
% surface(g_1BC, p_1BC);
xlabel('x'); ylabel('y'); zlabel('z');
view(30,40); axis('equal');
...
figure(3); clf;
% surface(g_AB1, p_AB1);
surface(g_A1C, p_A1C - real(P_ABC(:,20,:)));
% surface(g_1BC, p_1BC);
xlabel('x'); ylabel('y'); zlabel('z');
view(30,40); axis('equal');
...
#+BEGIN_SRC octave  :tangle sfs_plane_test.m :results output
vm_E = vectorgrid_vector(gg_CAB); % a grid in space
vk_F = vector_scale(k_F, vector_normalise(vM_src)); % wave number vector
